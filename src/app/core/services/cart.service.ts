...
export class CartService {
  ...
  loadCart() {
    return this.http.get<Cart>(this.base).pipe(
      tap((cart) => { this.cart.set(cart); this.itemCount.set(cart.items.length); })
    );
  }

  addItem(productId: string, quantity: number) {
    return this.http.post<Cart>(`${this.base}/items`, { productId, quantity }).pipe(
      tap((cart) => { this.cart.set(cart); this.itemCount.set(cart.items.length); })
    );
  }
  ...
  clearCart() {
    return this.http.delete(this.base).pipe(
      tap(() => { this.cart.set(null); this.itemCount.set(0); })
    );
  }
}
